export const COMPANY = {
  name: "Özlem İnşaat",
  fullName: "ÖZLEM İNŞAAT VE ALTYAPI ELEMANLARI TİC. LTD. ŞTİ.",
  brandName: "Özlem Boru",
  tagline: "Beton Boru ve Beton Elemanları Sanayi",
  slogan: "Modern şehirlerin güçlü alt yapılara ihtiyacı vardır. Biz bunun için varız...",
  founded: "1989",
  diameterRange: "Ø150–2800mm",
  gm: "Bülent Zafer ACUR",
  gmTitle: "Genel Müdür",
  address: "Yenice Mah. Yolüstü Küme Ev. No:6/A, Çubuk / ANKARA",
  phone1: "(0312) 354 64 84",
  phone2: "(0312) 385 27 77",
  /** Cep (teklif / iletişim blokları) */
  mobile: "0532 766 72 43",
  email: "info@ozlemboru.com",
  website: "www.ozlemboru.com",
};

/** Header logo (Next/Image width×height gerçek piksel oranı) */
export const SITE_LOGO = {
  src: "/images/ozlemlogo6.png",
  width: 2880,
  height: 900,
} as const;

/** Footer logo — mavi zemin için ayrı varyant */
export const FOOTER_LOGO = {
  src: "/images/ozlemfooterlogo.png",
  width: 2880,
  height: 900,
} as const;

/** Anasayfa hero arka plan videosu */
export const HERO_VIDEO = {
  src: "/ozlemherofinalsonopt.mp4",
  poster: "/heroyertutucu.png",
} as const;

export const NAV_LINKS = [
  { label: "Anasayfa", href: "/" },
  { label: "Kurumsal", href: "/kurumsal" },
  { label: "Ürünlerimiz", href: "/urunler" },
  { label: "Bilgi", href: "/bilgi" },
];

export const STATS = [
  { value: "35+", label: "Yıl Deneyim" },
  { value: "Ø150–2800mm", label: "Çap Aralığı" },
  { value: "TSE & ISO", label: "Belgeli Üretim" },
  { value: "60+", label: "Referans Proje" },
];

export const ABOUT_TEXT = `ÖZLEM İNŞAAT VE ALTYAPI ELEMANLARI TİC. LTD. ŞTİ., Beton Boru ve Beton Elemanları üretimi ve satışı amacıyla 1989 yılında ÖZLEM TİCARET olarak kurulmuştur. Aile şirketinin verdiği birlik, beraberlik ve gayretli çalışmalar sonucunda 1996 yılında bugünkü ünvanını almıştır.

Teknolojideki gelişmeleri yakından takip eden firmamız, kalite standartlarını sürekli yükseltmiştir. TSE tarafından verilen TS 821 EN 1916, TS EN 1917, TS 13473 belgelerini; İLLER BANKASI ve diğer kurumlarca verilen KALİTE, YETERLİLİK-İZİN belgelerini; T.C. Çevre ve Şehircilik Bakanlığı Çevre izinlerini ve ISO 9001–14001 Belgesini almış bulunmaktayız.

Faaliyet gösterdiğimiz alanda Ø150mm çaptan Ø2800mm'ye kadar Buhar Kürlü, Entegre Contalı/Contasız Beton-Betonarme Boru ve Muayene Bacaları üretimini gerçekleştirerek bölgenin önemli üretici firması olmanın haklı gururunu yaşıyoruz.`;

export const KFS_ITEMS = [
  {
    letter: "K",
    title: "Kaliteli Ürün",
    description:
      "TSE ve ISO standartlarına uygun, buhar kürlü üretim süreçleriyle en yüksek kalite güvencesi.",
  },
  {
    letter: "F",
    title: "Fiyat Uygunluğu",
    description:
      "Yerli hammadde kullanımı ve verimli üretim yapısıyla rekabetçi ve sürdürülebilir fiyat politikası.",
  },
  {
    letter: "S",
    title: "Sevk Zamanlaması",
    description:
      "Kamu kurumu ve özel teşebbüslerimize zamanında teslimat garantisiyle proje takviminize uyum.",
  },
];

export const PRODUCTS = [
  {
    id: "beton-betonarme-borular",
    title: "Beton ve Betonarme Borular",
    description:
      "Ø150mm ile Ø2800mm arasında, buhar kürlü, entegre contalı beton ve çelik takviyeli betonarme borular. Kanalizasyon ve drenaj hatlarında uzun ömürlü çözüm.",
    href: "/urunler/beton-betonarme-borular",
    hoverImage: "beton-betonarme-borular-hover.png",
  },
  {
    id: "muayene-bacasi-elemanlari",
    title: "Muayene Bacası Elemanları",
    description:
      "TS EN 1917 standardına uygun taban elemanları, gövde bilezikleri ve konik elemanlar. Kentsel altyapı sistemlerinde güvenli erişim ve denetim.",
    href: "/urunler/muayene-bacasi-elemanlari",
    hoverImage: "muayene-bacasi-elemanlari-hover.png",
  },
  {
    id: "parsel-bacasi-elemanlari",
    title: "Parsel Bacası Elemanları",
    description:
      "TS 13473 standardına uygun, konut ve sanayi parsellerinde kanalizasyon bağlantı noktaları için üretilen dayanıklı taban ve gövde elemanları.",
    href: "/urunler/parsel-bacasi-elemanlari",
    hoverImage: "parsel-bacasi-elemanlari-hover.png",
  },
  {
    id: "yagmur-suyu-izgara-elemanlari",
    title: "Yağmur Suyu Izgara Elemanları",
    description:
      "Yol ve kaldırım yüzey sularının toplanması için üretilen beton ızgara elemanları. Kentsel drenaj altyapısında yüksek taşıma kapasitesi.",
    href: "/urunler/yagmur-suyu-izgara-elemanlari",
    hoverImage: "yagmur-suyu-izgara-elemanlari-hover.png",
  },
  {
    id: "kanalizasyon-beton-ek-parcalari",
    title: "Kanalizasyon Beton Ek Parçaları",
    description:
      "Boru hatlarının birleşim noktalarında kullanılan beton ek parçaları. Sızdırmazlık ve uzun ömür için özel üretim.",
    href: "/urunler/kanalizasyon-beton-ek-parcalari",
    hoverImage: "kanalizasyon-beton-ek-parcalari-hover.png",
  },
  {
    id: "betonarme-su-depolari",
    title: "Betonarme Su Depoları",
    description:
      "İçme suyu ve sulama amaçlı, farklı kapasitelerde betonarme su depoları. Dayanıklı yapısı ve sızdırmazlık özellikleriyle uzun yıllar sorunsuz kullanım.",
    href: "/urunler/betonarme-su-depolari",
  },
];

export const WHY_CONCRETE = [
  {
    icon: "flag",
    title: "Yerli Üretim",
    description:
      "Yalnızca kum, çimento ve su kullanılarak üretilir. Hammadde ve üretim ekipmanları tamamen yerlidir; milli sermaye ülke sınırları içinde kalır.",
  },
  {
    icon: "leaf",
    title: "Çevre Dostu",
    description:
      "Doğal malzemelerden üretilir, geri dönüştürülebilir. Üretiminde 1 kg koruge boruya karşın 1,75 kg petrol gerekirken beton boru için petrol kullanılmaz.",
  },
  {
    icon: "shield",
    title: "Uzun Ömürlü",
    description:
      "Yüzyılı aşkın süredir kanalizasyon şebekelerinde kanıtlanmış dayanıklılık. Her bölgede 120'nin üzerinde ülkede kabul görmüş güvenilir çözüm.",
  },
  {
    icon: "zap",
    title: "Yüksek Basınç Dayanımı",
    description:
      "Aşınma ve yük basıncına üstün direnç. Özel inşaatlar ve yoğun trafik yüküne karşı kimyasal şartlara tam uyumlu yapı.",
  },
  {
    icon: "trendingDown",
    title: "Ekonomik",
    description:
      "Düşük üretim maliyeti ve uzun kullanım ömrü sayesinde toplam yaşam döngüsü maliyeti en düşük altyapı ürünü. Bakım giderleri minimumdur.",
  },
  {
    icon: "globe",
    title: "Evrensel Standart",
    description:
      "ABD ve Avrupa Beton Birliklerinin (ACPA, ECPA) bilimsel çalışmalarıyla doğrulanan global standartlara uygunluk. 120+ ülkede tercih edilen güvenilir çözüm.",
  },
];

export const CERTIFICATES = [
  {
    code: "TS 821 EN 1916",
    title: "Beton Boru",
    issuer: "TSE",
    description: "Betonarme ve düz beton borular için Türk-Avrupa standardı",
  },
  {
    code: "TS EN 1917",
    title: "Muayene Bacası",
    issuer: "TSE",
    description: "Beton muayene bacaları ve denetleme odaları standardı",
  },
  {
    code: "TS 13473",
    title: "Parsel Bacası",
    issuer: "TSE",
    description: "Beton parsel bacası elemanları için Türk standardı",
  },
  {
    code: "ISO 9001",
    title: "Kalite Yönetimi",
    issuer: "ISO",
    description: "Uluslararası kalite yönetim sistemi sertifikası",
  },
  {
    code: "ISO 14001",
    title: "Çevre Yönetimi",
    issuer: "ISO",
    description: "Çevresel etkilerin yönetimi ve kirlenmenin önlenmesi",
  },
  {
    code: "KALİTE & YETERLİLİK",
    title: "İller Bankası",
    issuer: "İLBANK",
    description: "İller Bankası A.Ş. kalite ve yeterlilik belgesi",
  },
];

export const REFERENCE_INSTITUTIONS = [
  "İLBANK",
  "ASKİ",
  "TOKİ",
  "Ankara Büyükşehir Belediyesi",
  "Çevre ve Şehircilik Bakanlığı",
  "Sanayi ve Ticaret Bakanlığı",
  "TSE",
];

export const REFERENCES = [
  "İller Bankası Genel Müdürlüğü",
  "İller Bankası Ankara Bölge Müdürlüğü",
  "İller Bankası Kastamonu Bölge Müdürlüğü",
  "İller Bankası Eskişehir Bölge Müdürlüğü",
  "İller Bankası Konya Bölge Müdürlüğü",
  "İller Bankası Samsun Bölge Müdürlüğü",
  "ASKİ Genel Müdürlüğü",
  "ASKİ Kanal Yatırım Dairesi Başkanlığı",
  "Ateşler İnşaat / Mamak AS-YS. Yapım İşi",
  "Ateş İnşaat",
  "Özdoğanlar İnşaat",
  "MSK Grup Yatırım A.Ş.",
  "Nata Holding",
  "MRM-Maksu İş Ortaklığı / Keçiören AS-YS. Yapım İşi",
  "Güvenç Yapı-Hasan Güvenç / Bolu-Bartın-Turgutlu DSB / Taşova Kanalizasyon İnşaatı",
  "Alken-Miracyol İş Ortaklığı / Mahalle Kanalizasyon Bakım Onarım İşi",
  "Yasmak-Koza İş Ortaklığı / Sincan AS-YS. Yapım İşi",
  "Alptekin İnşaat / OSB İnşaatı",
  "Özkar İnşaat A.Ş. (TOKİ-Ankara/Rize/Van)",
  "Akay İnşaat A.Ş. (TOKİ-Ankara/Kutahya/Van)",
  "YDA İnşaat / Nevbahar Avanue, İncek Altyapı İnşaatı",
  "Mimaray İnşaat / Kilimli (Zonguldak) Kanalizasyon İnşaatı",
  "Fetvacı Kardeşler / Çerkeş (Çankırı) Kanalizasyon İnşaatı",
  "Sürekli İnşaat / Şabanozu (Çankırı) Kanalizasyon İnşaatı",
  "Sürekli İnşaat / Kandil-Kolukisa (Konya) Kanalizasyon İnşaatı",
  "Mehmet Baki Duvan / Perşembe (Zonguldak) Kanalizasyon İnşaatı",
  "Mehmet Baki Duvan / Gökçesu (Bolu) Kanalizasyon İnşaatı",
  "Nuri Akgül İnşaat / Sulakyurt (Kırıkkale) Kanalizasyon İnşaatı",
  "Dağdemir İnşaat / Çeltikçi (Kıbrıs Köyü) Kanalizasyon İnşaatı",
  "Mehmet Gülşen İnşaat / Eskipazar (Karabük) Kanalizasyon İnşaatı",
  "Ercan Eker İnşaat / Azdavay (Kastamonu) Kanalizasyon İnşaatı",
  "Esen İnşaat / Taşköprü (Kastamonu) Atıksu Arıtma Tesisi İnşaatı",
  "Hasan Yüksel İnşaat / Pazarköy (Mengen) Kanalizasyon İnşaatı",
  "Eflanı Belediye Başkanlığı",
  "Mengen Belediye Başkanlığı",
  "Çerkeş Belediye Başkanlığı",
  "Şabanozu Belediye Başkanlığı",
  "Gölbaşı Belediye Başkanlığı",
  "Sungurlu Belediye Başkanlığı",
  "Emirdağ Belediye Başkanlığı",
  "Ahmet Aydeniz İnşaat",
  "Aras Kardeşler",
  "Başyapı İnşaat (TOKİ-Çankırı / Karabük / Eskişehir)",
  "Besa İnşaat A.Ş.",
  "BM Holding (Jokey Kulübü İnşaatı)",
  "Bilir İnşaat",
  "Ekşioğlu İnşaat",
  "HBV İnşaat / Balçikhisar (Haymana) Kanalizasyon İnşaatı",
  "Gayem İnşaat / Çerkes OSB. İnşaatı",
  "Türkerler İnşaat (TOKİ-Kuzey Ankara)",
  "Menka Mühendislik",
  "MFZ Gözde Yapı-Gözüm İnşaat",
  "Öz Doğuz İnşaat (TOKİ-Kuzey Ankara)",
  "Can İnşaat (TOKİ-Kuzey Ankara)",
  "Varyap Varlıbaşlılar (Ankara Şantiyeleri)",
  "Yapı Proje A.Ş.",
  "Tepe İnşaat / Bilkent İnşaatı Konya Yolu İnşaatı",
  "S.S. Saraykent Yapı Koop / Kazan-Sanayi Bölgesi",
  "S.S. Koop18 Yapı Koop (Angora Evleri)",
  "Uçural A.Ş.",
  "Sentez İnşaat",
  "Bir İnşaat A.Ş.",
];

export const SEO = {
  title:
    "Özlem Beton Boru | Ankara Beton Boru ve Altyapı Elemanları Üreticisi",
  description:
    "Çubuk/Ankara merkezli, 1989'dan bu yana TS 821 EN 1916 belgeli beton boru, betonarme boru ve altyapı elemanları üreticisi. Ø150–2800mm. TSE & ISO 9001-14001 sertifikalı.",
  keywords: [
    "beton boru",
    "betonarme boru",
    "beton boru Ankara",
    "beton boru Çubuk",
    "muayene bacası",
    "parsel bacası",
    "altyapı elemanları",
    "entegre contalı beton boru",
    "TS 821 EN 1916",
    "beton büz",
    "kanalizasyon borusu",
    "drenaj borusu",
    "Özlem İnşaat",
    "Özlem Boru",
  ],
  ogImage: "/images/og-image.jpg",
  siteUrl: "https://www.ozlemboru.com",
};
